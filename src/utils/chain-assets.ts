 ...
 */
export const CHAIN_TO_CAIP2: Record<Chain, string> = {
  base: "eip155:8453",
  ethereum: "eip155:1",
 ...
 */
export function assetFor(chain: Chain, token: Token): string {
  if (token === "USDC") return USDC_ADDRESS[chain];
  return token;
}
 ...
 */
export function toAtomicString(amount: number, token: Token): string {
 ...
 */
export function creditsFor(humanAmount: number, token: Token): number {
 ...
 */
export function creditsFromAtomicUsdc(atomicAmount: string): number {
